...
public partial class RecordsEditorWindow : RibbonWindow
{
    ...
    private async void GetMappingDetails()
    {
        try
        {
            var response = await App.HttpClient.GetAsync(App.ApiBaseUrl + "api/Mapping/GetMappingDetails");
            response.EnsureSuccessStatusCode();
            _mappingDetails = await response.Content.ReadFromJsonAsync<MappingDetails>() ?? new MappingDetails();
        }
    ...
    private async void Unmap(Mapping mapping)
    {
        ...
        try
        {
            (await App.HttpClient.PostAsync(App.ApiBaseUrl + "api/Mapping/UnMap?MappingId=" + mapping.MappingId, null))
    ...
    private void rbn_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        var mapButton = new RibbonButton
        {
        ...
        var mapExplorerButton = new RibbonButton
        {
            SizeForm = SizeForm.Large,
            Label = "Mappings Explorer"
        };
        ...
        var verifyButton = new RibbonButton
        {
            SizeForm = SizeForm.Large,
    ...
    private void sp_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        {
            e.Handled = true;
            if (MessageBox.Show("Are you sure to unmap this column?", "Unmap confirmation", MessageBoxButton.YesNo, MessageBoxImage.Question) != MessageBoxResult.Yes)
            {
                return;
            }
    ...
    private void btnMappingExplorer_Click(object sender, RoutedEventArgs e)
    {
        ...
        var mappingExplorerWindow = new MappingExplorerWindow(_mappingDetails)
        {
            Owner = this
        };
